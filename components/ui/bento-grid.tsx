...
const BentoCard = ({
  ...
  <div
    key={name}
    className={cn(
      "group relative col-span-1 flex flex-col justify-between overflow-hidden rounded-xl text-white",
      // light styles
  ...
  >
    ...
    <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 p-6 transition-all duration-300 group-hover:-translate-y-10">
      <div className="flex items-center gap-2 mb-2">
        ...
        {popular && (
          <span className="px-3 py-1 rounded-full bg-white/20 backdrop-blur-sm text-white text-xs font-bold shadow border border-white/30">
            Most Popular
          </span>
        )}
      </div>
    ...
    >
      <Button variant="ghost" asChild size="sm" className="pointer-events-auto text-white hover:bg-white/20 border-white/30">
        <a href={href}>
          {cta}
