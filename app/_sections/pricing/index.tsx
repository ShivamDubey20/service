...
export function Pricing(pricing: Pricing) {
  return (
    <Section className="xl:max-w-screen-xl" id="pricing">
      ...
      <div className="flex flex-col gap-6 self-stretch lg:flex-row">
...
function PricingCard(item: PricingPlanItem["plan"]) {
  return (
    <article
      key={item._title}
      className={clsx(
        "relative flex flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-white to-gray-50 dark:from-gray-900 dark:to-gray-800 hover:shadow-xl hover:shadow-blue-500/10 dark:hover:shadow-purple-500/10 transition-all duration-300 transform hover:-translate-y-1 dark:border-dark-border",
        s.pricingCard,
        item.isMostPopular && "ring-2 ring-gradient-to-r from-blue-500 to-purple-500 shadow-lg"
      )}
    >
      {/* Gradient overlay for popular plan */}
      {item.isMostPopular && (
        <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 via-purple-500/3 to-pink-500/5 rounded-2xl" />
      )}
      
      <header className="relative flex flex-col gap-4 px-8 pb-0 pt-10">
        {item.isMostPopular ? (
          <span className="bg-gradient-to-r from-blue-500 to-purple-500 absolute left-1/2 top-4 -translate-x-1/2 text-center text-xs font-medium text-white px-4 py-1 rounded-full shadow-lg lg:text-sm">
            Most popular
          </span>
        ) : null}
        <span className="text-center text-3xl font-medium lg:text-4xl bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-200 bg-clip-text text-transparent">
          {item.price}
        </span>
        <div className="flex flex-col">
          <h5 className="text-center text-lg font-medium lg:text-xl bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-200 bg-clip-text text-transparent">
            {item._title}
          </h5>
      ...
      <div className="relative flex flex-1 flex-col gap-6 p-6 !pb-12 lg:p-8">
        <ul className="flex flex-col gap-4">
          {item.list.items.map((feature) => (
            ...
            >
              <div className="mt-0.5 size-4 shrink-0 lg:size-5 bg-gradient-to-r from-green-500 to-emerald-500 rounded-full flex items-center justify-center">
      ...
      <footer className="relative flex w-full items-center self-stretch p-8 pt-0">
        {item.isMostPopular ? (
          <Shadow className="pointer-events-none absolute left-0 top-0 h-full w-full origin-bottom scale-[2.0] text-blue-500" />
        ) : null}
        <ButtonLink
